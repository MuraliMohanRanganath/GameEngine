...
namespace Engine
{
	...
	void SpriteRenderer::run()
	{
		...
		while (spriteRendererIterator != SpriteRenderer::spritesToRender.end())
		{
			...
			GLib::Sprites::RenderSprite(*(spriteInfo->sprite), Offset,spriteInfo->object->getRotation() * 3.14f / 180.0f);
	...
	{
		isInitiliazed = GLib::Initialize(i_hInstance, i_nCmdShow, "UFO Killer", -1, 800, 600);
